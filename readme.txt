...
Using Windows Explorer
    ...
    Navigate to the samsmovieclub folder on the drive.
    Double click on the movieclub.bat file to run samsmovieclub.py file.
    The default browser should open with the samsMovieClub.html page.
...
Assumptions
    ...
    All data used is for demonstration only and completely fictitious.
    All user names used and rating are for demonstration only.
    In this version of the code the only way to add media and media reviews is through the csv files mentioned in this document.
    Data is validated before hand ehen entring it in csv files.
...
Program FLow
    ...
    The code first reads the mediareviews.csv and loads all the reviews in a list all_reviews.
    The code then parses the mediadetails.csv and for each row decides if it is a movie or a TV show. A appropreate instance of Movie or the TVShow class is created.
    The code reads the all_reviews and gets and adds all the reviews for the current media.
    The instance is then added to the movie_list or the shows_list as per media_type.
    Once all the rows are read the fresh_tomatoes is called to parse and create the list
